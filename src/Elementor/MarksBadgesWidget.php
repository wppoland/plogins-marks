...
final class MarksBadgesWidget extends Widget_Base
{
    ...
    {
        return esc_html__('Product Badges', 'plogins-sale-stock-badges');
    }
    ...
    protected function register_controls(): void
    {
        $this->start_controls_section(
            'content',
            ['label' => esc_html__('Product Badges', 'plogins-sale-stock-badges')]
        );

        $this->add_control(
            'product_id',
            [
                'label'       => esc_html__('Product ID', 'plogins-sale-stock-badges'),
                'type'        => Controls_Manager::NUMBER,
                'default'     => 0,
                'min'         => 0,
                'description' => esc_html__('Leave 0 to use the current product on a product page.', 'plogins-sale-stock-badges'),
            ]
        );
